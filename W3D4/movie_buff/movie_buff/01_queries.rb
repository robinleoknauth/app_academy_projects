def it_was_ok
  ...
  # Movie.where('yr BETWEEN 1970 AND 1979')
  Movie.select(:title,:id, :score).where(score: 2.0..3.0)
    # Movie.where('score = 3.0')#.select(:id, :title, :score)
end
...
def directed_by_one_of(them)
  ...
  Movie
    ...
    # where('acotrs.id = castings.directors_id')
